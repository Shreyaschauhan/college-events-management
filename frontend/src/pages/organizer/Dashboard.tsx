...
import { Event, User /* Removed Enrollment import as it's not directly used here */ } from '@/lib/types'; // Assuming User has _id, fullName, email, profileImage?
...
// Removed enrollmentService import as we now use eventService for participants
// import { enrollmentService } from '@/services/enrollmentService';

// Define a type for participant data shown in the dialog
// Matches the expected output from eventService.getEventParticipants (based on backend controller)
type ParticipantDetails = {
  _id: string;
  fullName: string;
  email: string;
  profileImage?: string; // Include if your backend User model has it and it's selected
};
...
const OrganizerDashboard = () => {
  ...
  const [participants, setParticipants] = useState<ParticipantDetails[]>([]); // Use updated type
  ...
  const handleCreateEvent = async (formData: Omit<Event, '_id' | 'id' | 'organizerId' | 'organizer' | 'status' | 'participants' | 'createdAt' | 'updatedAt'>) => {
    if (!user) return;
